...
def compute_pred(output, device):
...
def get_attr(model, image, target_class, device, saver, baselines, lrp, segment_count, index, threshold, kappa, blur, labels):
    if (args.method == 'IG'):    
    ...
    elif args.method == 'Calibrate_Best_Possible':
        trans_img = inv_normalize(image.squeeze())
        segment_img = np.transpose(trans_img.squeeze().detach().cpu().numpy(), (1, 2, 0))
        segment_img = img_as_float(segment_img)
        segments = torch.tensor(slic(segment_img, n_segments=segment_count, compactness=10000, start_label=0), dtype = int)
        ...
        _, _, order_a, MR_ins = MASCalibrate.find_best_segment_reverse_informed(image.cpu(), saliency_map_segmented, segments, blur, segment_count, type = 1, model = model, device = device, img_hw = 224, max_batch_size = 25)
        end_index = np.where(MR_ins >= 0.9)[0][0]
        _, Res = MASCalibrate.find_deletion_from_insertion_informed_ultimate(image.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, segment_count, model, device, 224, max_batch_size = batch_size, kappa = -1)
    ...
    if 'Calibrate_Best_Possible' in args.method:
...
def eval_batch(model, device, saver, baselines, lrp, segment_count, args, image, labels, index, acc_cutoff, threshold, kappa):
    ...
    Res, ret = get_attr(model, image, target_class, device, saver, baselines, lrp, segment_count, index, threshold, kappa, blur, labels)
...
def main(args):
    ...
    # Define Saver
    args.checkname = args.model + '/' + args.method
    saver = Saver(args)
    ...
    ds = Imagenet_Segmentation(args.imagenet_seg_path, transform=test_img_trans, target_transform=test_lbl_trans)
    subset_indices = list(range(0, 100)) # First 100 samples
    ...
    for batch_idx, (image, labels) in enumerate(iterator):
        ...
        correct, labeled, inter, union, ap, f1, pred, target = eval_batch(model, device, saver, baselines, lrp, segment_count, args, images, labels, batch_idx, args.acc_cutoff, args.threshold, args.kappa)
