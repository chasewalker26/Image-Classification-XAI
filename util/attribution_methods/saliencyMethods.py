...
# This file implements IG (LIG), IDG, IDGI, and IG, LIG, IDG smoothgrad
# For the attributions, the input is a normalized tensor image (1, 3, 224, 224)


def input_grad(input, model, target_class):
    input.requires_grad = True
    gradient, _ = getGradientsParallel(input, model, target_class)
...
def IDG(input, model, steps, batch_size, baseline, device, target_class):
    ...
    return grads.squeeze().detach()

# Adapted from https://github.com/yangruo1226/IDGI/blob/main/idgi.py
def IDGI(input, model, steps, batch_size, baseline, device, target_class):
    if (steps % batch_size != 0):
        print("steps must be evenly divisible by batch size: " + str(batch_size) + "!")
        return 0, 0, 0, 0
    ...
    # compute idgi given gradients and logits of an IG function
    idgi_result = torch.zeros_like(gradients[0])
    for i in range(len(gradients) - 1):
        ...
        d = logits[i + 1] - logits[i]
        element_product = gradients[i] ** 2
        idgi_result += element_product * d / torch.sum(element_product)

    return idgi_result

# Adapted from https://github.com/PAIR-code/saliency/blob/master/saliency/core/base.py
def smoothGrad(attribution, input, model, steps, baseline, target_class, device, sigma_spread = .15, samples = 25, vis = False):
    stdev = sigma_spread * (torch.max(input) - torch.min(input))

    total_gradients = torch.zeros((samples, input.shape[1], input.shape[2], input.shape[3]))
    noisy_imgs = torch.zeros((samples, input.shape[1], input.shape[2], input.shape[3]))

    for i in range(samples):
        noise = torch.normal(mean = 0, std = stdev, size = input.shape)

        noisy_imgs[i] = input + noise.to(device)

        if attribution == "IG":
            total_gradients[i], _, _= IG(noisy_imgs[i].unsqueeze(0), model, steps, int(steps / 2), 1, baseline, device, target_class)
        elif attribution == "LIG":
            total_gradients[i], _, _ = IG(noisy_imgs[i].unsqueeze(0), model, steps, int(steps / 2) / 2, .9, baseline, device, target_class)
        elif attribution == "IDG":
            total_gradients[i], _, _  = IDG(noisy_imgs[i].unsqueeze(0), model, steps, int(steps / 2) / 2, baseline, device, target_class)

    if vis == False:
        return total_gradients.mean(dim = 0)
    elif vis == True:
        return total_gradients.mean(dim = 0), total_gradients, noisy_imgs
...
def getAlphaParameters(slopes, steps, step_size):
    # normalize slopes 0 to 1 to preserve magnitude
