...
def find_insertion_patches(input_tensor, saliency_map_segmented, segments, blur, n_searches, type, model, device, img_hw, max_batch_size = 25, cutoff = 0.9, CLIP_test_info = None):
    if cutoff == 0:
    ...
    if CLIP_test_info is None:
        ...
        target_class, original_pred = pred_func(input_tensor.to(device), model, CLIP_embeddings)

        # deletion
        if type == 0:
            start = torch.zeros_like(input_tensor).to(device)
            finish = input_tensor.clone()
            _, black_pred = pred_func(finish, model, CLIP_embeddings)
        # insertion
        elif type == 1:
            start = blur(input_tensor.cpu()).to(device)
            finish = input_tensor.clone()
            _, blur_pred = pred_func(start, model, CLIP_embeddings)
    ...
    for step in range(n_searches - subsearch_len):
        ...
        for batch in batches:
            ...
            if CLIP_test_info is None:
                ...
                img_embedding = model.encode_image(images.to(device))
                similarities = img_embedding @ CLIP_embeddings.squeeze().T
                model_response[total_steps - batch : total_steps] = torch.nn.functional.softmax(similarities / 0.1, dim=-1)[:, target_class]
...
def find_deletion_patches(input_tensor, segments, saliency_map_segmented, beginning_order, blur, n_searches, model, device, img_hw, max_batch_size = 25, kappa = 0.005, test_kappa = False, CLIP_test_info = None):
    n_steps = len(np.unique(segments))
    ...
    if CLIP_test_info is None:
        ...
        start = torch.zeros_like(input_tensor)
        finish = input_tensor.clone()
        _, black_pred = pred_func(finish.to(device), model, CLIP_embeddings)
    ...
    for step in range(worst_segment_list_length - input_length , worst_segment_list_length):
        ...
        if CLIP_test_info is None:
            ...
            img_embedding = model.encode_image(start.to(device))
            similarities = img_embedding @ CLIP_embeddings.squeeze().T
            model_response = torch.nn.functional.softmax(similarities / 0.1, dim=-1)[:, target_class]
            worst_MR_list[step] = model_response
    ...
    if CLIP_test_info is None:
        _, _, _, _, raw_score_ins = MAS_insertion.single_run(input_tensor, new_map_test, device, max_batch_size=max_batch_size)
        _, _, _, _, raw_score_del = MAS_deletion.single_run(input_tensor, new_map_test, device, max_batch_size=max_batch_size)
    else:
        _, _, _, _, raw_score_ins = MAS_insertion.single_run(input_tensor.cpu(), new_map_test, device, max_batch_size=max_batch_size, CLIP_test_info = CLIP_test_info)
        _, _, _, _, raw_score_del = MAS_deletion.single_run(input_tensor.cpu(), new_map_test, device, max_batch_size=max_batch_size, CLIP_test_info = CLIP_test_info)
...
def MDA(trans_img, input_tensor, saliency_map, patch_count, blur, model, device, img_hw, max_batch_size = 5, ordered = False, CLIP_test_info = None, vis = False):
    ...
    upsize = transforms.Resize((img_hw, img_hw), interpolation=transforms.InterpolationMode.NEAREST_EXACT, antialias=True)
    downsize = transforms.Resize(int(patch_count ** (1/2)), antialias=True)
    ...
    _, _, order_a, MR_ins = find_insertion_patches(input_tensor.cpu(), saliency_map_segmented, segments, blur, patch_count, type = 1, model = model, device = device, img_hw = img_hw, max_batch_size = max_batch_size, CLIP_test_info = CLIP_test_info)
    
    try:
        end_index = np.where(MR_ins >= 0.9)[0][0]
    except:
        end_index = len(MR_ins)
    if ordered == False:
        MGA_g_0, MGA_g_0_s, MGA_g_5, MGA_g_5_s, MGA_g_10, MGA_g_10_s,  _ = find_deletion_patches(input_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, patch_count, model, device, img_hw, max_batch_size = max_batch_size, CLIP_test_info = CLIP_test_info)
    else:
        MGA_g_0, MGA_g_0_s, MGA_g_5, MGA_g_5_s, MGA_g_10, MGA_g_10_s,  _ = find_deletion_patches(input_tensor.cpu(), segments, saliency_map_segmented, order_a[0 : end_index + 1], blur, patch_count, model, device, img_hw, max_batch_size = max_batch_size, kappa = -1, CLIP_test_info = CLIP_test_info)

    if vis == False:
        return MGA_g_0, MGA_g_0_s, MGA_g_10
    else:
        return MGA_g_0, MGA_g_0_s, MGA_g_5, MGA_g_5_s, MGA_g_10, MGA_g_10_s