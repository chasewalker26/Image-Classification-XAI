...
from .Game_MM_CLIP import clip as mm_clip
from .CLIP_Surgery import clip as surgery_clip
from .CLIP_lrp.CLIP.clip import clip as lrp_clip
...
# lrp_clipmodel, _ = lrp_clip.load("ViT-B/16", device=device, jit=False)
...
# lrp_clipmodel, _ = lrp_clip.load("ViT-B/32", device=device, jit=False)
...
### M2IB
def m2ib_clip_map(model, clip_tokenizer, image, texts, device, vbeta=0.1, vvar=1, vlayer=9, tbeta=0.1, tvar=1, tlayer=9):
    text_ids = torch.tensor([clip_tokenizer.encode(texts, add_special_tokens=True)]).to(device)
    vmap = vision_heatmap_iba(text_ids, image, model, vlayer, vbeta, vvar, progbar=False, device = device)
    return vmap

def m2ib_clip_text_map(model, clip_tokenizer, image, texts, device, vbeta=0.1, vvar=1, vlayer=9, tbeta=0.1, tvar=1, tlayer=9):
    text_ids = torch.tensor([clip_tokenizer.encode(texts, add_special_tokens=True)]).to(device)
...
def clip_lrp(image, texts, model, device, start_layer=0, start_layer_text=0):
    batch_size = texts.shape[0]
    images = image.repeat(batch_size, 1, 1, 1)
    logits_per_image, logits_per_text = model(images, texts)
    probs = logits_per_image.softmax(dim=-1).detach().cpu().numpy()
...
def clip_encode_text_dense(text, n, clipmodel):
...
def grad_eclip(c, q_out, k_out, v, att_output, map_size, withksim=True, withgrad = True):
    D = k_out.shape[-1]
    if withgrad == True:
        ## gradient on last attention output
        grad = torch.autograd.grad(
        ...
        if withksim:
            ...
            cosine_qk = (q_cls * k_patch).sum(-1) 
            cosine_qk = (cosine_qk-cosine_qk.min()) / (cosine_qk.max()-cosine_qk.min())
            emap_lastv = F.relu_((v[1:,0,:] * cosine_qk[:,None]).detach().sum(-1)) # 
        else:
            emap_lastv = F.relu_((v[1:,0,:]).detach().sum(-1)) 
