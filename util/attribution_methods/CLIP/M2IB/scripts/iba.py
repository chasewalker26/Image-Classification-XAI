...
from .utils import replace_layer, normalize, mySequential
...
class InformationBottleneck(nn.Module):
    def __init__(self, mean: np.ndarray, std: np.ndarray, device=None):
        ...
        # self.mean = mean.clone().detach().to(dtype=torch.float, device=self.device)
        self.mean = torch.tensor(mean.detach().cpu().numpy(), dtype=torch.float, device=self.device, requires_grad=False)
...
class IBAInterpreter:
    def __init__(self, model, estim: Estimator, beta, steps=10, lr=1, batch_size=10, progbar=False, device = "cuda:0"):
        self.device =device
