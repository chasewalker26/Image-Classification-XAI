...
def compute_RAVE(all_layer_attentions, all_layer_biases_resid_1, all_layer_biases_resid_2, ablate = 0):
...
class Baselines:
    ...
    def generate_raw_attn(self, input, device, layer = -1):
        self.model(input.to(device))
        attn = self.model.blocks[layer].attn.get_attention_map().mean(1)
        attn = attn[0, 0, 1:]
        patches_per_side = int(np.sqrt(attn.shape[-1]))
        return attn.reshape(-1, patches_per_side, patches_per_side)

    def generate_grad(self, input, target_class, device, layer = -1):
        ...
        grad = self.model.blocks[layer].attn.get_attn_gradients().mean(1)[:, 0, 1:].clamp(0)

        patches_per_side = int(np.sqrt(grad.shape[-1]))

        return grad.reshape(-1, patches_per_side, patches_per_side)
    
    # https://github.com/hila-chefer/Transformer-Explainability/blob/main/baselines/ViT/ViT_explanation_generator.py
    def generate_cam_attn(self, input, target_class, device, layer = -1):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook=True)
        score = output[0][target_class].sum()
        score.backward()
        ...
        grad = self.model.blocks[layer].attn.get_attn_gradients()
        cam = self.model.blocks[layer].attn.get_attention_map()

        patches_per_side = int(np.sqrt(grad.shape[-1] - 1))

        grad = grad[0, :, 0, 1:].reshape(-1, patches_per_side, patches_per_side)
        cam = cam[0, :, 0, 1:].reshape(-1, patches_per_side, patches_per_side)
    ...
    def generate_rollout(self, input, InFlow = False, start_layer = 0):
        self.model(input)
        blocks = self.model.blocks
        all_layer_attentions = []
        ...
        if InFlow == False:
            ...
            rollout, test = compute_RAVE(all_layer_attentions, all_layer_biases_resid_1, all_layer_biases_resid_2)
        rollout = rollout[:, 0, 1:]
        patches_per_side = int(np.sqrt(rollout.shape[-1]))

        return rollout.reshape(-1, patches_per_side, patches_per_side), test, torch.stack(all_layer_attentions)

    def generate_RAVE(self, input, target_class, withgrad = True, device = "cuda:0", ablate = 0, target_token = 0, stop_layer = 12):
        blocks = self.model.blocks
        b, num_heads, num_tokens, _ = self.model.blocks[-1].attn.get_attention_map().shape
        _, _, embed_dim = self.model.blocks[-1].attn.get_output().shape
        embed_per_head = int(embed_dim / num_heads)
        ...
        for i, blk in enumerate(blocks[0 : stop_layer + 1]):
            ...
            input = blk.get_input().squeeze()
            qkv = blk.attn.get_qkv_res().squeeze()
            attn = blk.attn.get_output().squeeze()
            resid_1 = blk.get_input_plus_attn().squeeze()
            mlp = blk.get_mlp_val().squeeze()
        ...
        rollout, _ = compute_RAVE(all_layer_attentions, all_layer_biases_resid_1, all_layer_biases_resid_2, ablate)
        rollout = rollout[:, target_token, 1:]

        patches_per_side = int(np.sqrt(rollout.shape[-1]))

        return rollout.reshape(-1, patches_per_side, patches_per_side), (torch.stack(all_layer_biases_resid_1), torch.stack(all_layer_biases_resid_2))
    ...
    def IG(self, input, target_class, steps = 20, device = "cuda:0"):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook = True)
        score = output[0][target_class].sum()
        score.backward()
    ...
    # https://github.com/yxheartipp/attr-rollout
    def attn_attr(self, input, target_class, start_layer=0, device = "cuda:0"):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook=True)
        score = output[0][target_class].sum()
        score.backward()
        ...
        return states[:, 0, 1:].reshape(-1, patches_per_side, patches_per_side)

    # https://github.com/jiaminchen-1031/transformerinterp/blob/master/ViT/baselines/ViT/ViT_explanation_generator.py
    def bidirectional(self, input, target_class, steps=20, start_layer=4, samples=20, noise=0.2, mae=False, dino=False, ssl=False, InFlow = False, device = "cuda"):
        input.requires_grad = True
        output = self.model(input.to(device), register_hook=True)
        score = output[0][target_class].sum()
        score.backward()
        ...
        if InFlow == False:
            ...
            for nb, blk in enumerate(self.model.blocks):
                ...
                input_0 = blk.get_input().squeeze()
                attn = blk.attn.get_output().squeeze()
                resid_1 = blk.get_input_plus_attn().squeeze()
                mlp = blk.get_mlp_val().squeeze()
                norm = 2
                all_layer_biases_resid_1 = torch.stack((torch.linalg.norm(input_0, ord = norm, dim = 1), torch.linalg.norm(attn, ord = norm, dim = 1)))
                all_layer_biases_resid_1 = torch.nn.functional.normalize(all_layer_biases_resid_1, p = 1, dim = 0)
                all_layer_biases_resid_2 = torch.stack((torch.linalg.norm(resid_1, ord = norm, dim = 1), torch.linalg.norm(mlp, ord = norm, dim = 1)))
                all_layer_biases_resid_2 = torch.nn.functional.normalize(all_layer_biases_resid_2, p = 1, dim = 0)
                matrices_aug_resid_1 = cam * all_layer_biases_resid_1[1].reshape(1, 1, num_tokens) + R * torch.diag_embed(all_layer_biases_resid_1[0].squeeze()).reshape(cam.shape)
                mlp_resid_1_ratio = all_layer_biases_resid_2[1].reshape(1, 1, num_tokens) / all_layer_biases_resid_2[0].reshape(1, 1, num_tokens)
                mlp_resid_1_ratio = torch.nn.functional.normalize(mlp_resid_1_ratio, p = 1, dim = -1)
                matrices_aug_resid_2 = torch.diag_embed(mlp_resid_1_ratio.squeeze()).reshape(cam.shape) * torch.diag_embed(all_layer_biases_resid_2[1].squeeze()).reshape(cam.shape) + R * torch.diag_embed(all_layer_biases_resid_2[0].squeeze()).reshape(cam.shape)
                R = matrices_aug_resid_1 @ matrices_aug_resid_2
