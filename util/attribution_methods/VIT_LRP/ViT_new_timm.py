...
class VisionTransformer(nn.Module):
    ...
    def save_patch_embeding(self, patch_embeding):
        self.patch_embeding = patch_embeding

    def get_patch_embeding(self):
        return self.patch_embeding
    ...
    def forward_features(self, x, register_hook = False):
        ...
        for blk in self.blocks:
            ...
            # get the classification from this layer and save to block_classification_probs
            y = self.norm(x)
            y = y.mean(dim = 1) 
            y = self.head(y)
...
@register_model
def vit_base_patch16_224_dino(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
