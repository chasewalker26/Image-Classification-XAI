...
class IntGradSG(object):
    def __init__(self, model, device, k, bg_size, random_alpha=False, est_method='vanilla', exp_obj='logit', dataset_name='imagenet'):
        self.model = model
        self.model.eval()
        self.device = device
        self.k = k
    ...
    def _get_samples_input(self, input_tensor, reference_tensor):
        ...
        if self.random_alpha:
            t_tensor = torch.FloatTensor(batch_size, k_*self.bg_size).uniform_(0, 1).to(self.device)
        else:
            if k_ == 1:
                t_tensor = torch.cat([torch.Tensor([1.0]) for _ in range(batch_size*self.bg_size)]).to(self.device)
            else:
                t_tensor = torch.cat([torch.linspace(0, 1, k_) for _ in range(batch_size*self.bg_size)]).to(self.device)
    ...
    def _get_grads(self, samples_input, sparse_labels=None):
        ...
        grad_tensor = torch.zeros(shape).float().to(self.device)
    ...
    def chew_input(self, input_tensor):
        ...
        reference_tensor = torch.stack(ref_lst, dim=0).to(self.device)
        
        print(reference_tensor.device)
    ...
    def shap_values(self, input_tensor, sparse_labels=None):
        ...
        if self.est_method == 'valid_ip':
            ...
            mult_grads = mult_grads.sum(1) / torch.where(counts == 0., torch.ones(counts.shape).to(self.device), counts)
