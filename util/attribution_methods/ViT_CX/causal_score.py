...
class causal_score(nn.Module):
    def __init__(self, model, input_size, gpu_batch=100, device = "cuda:0"):
    ...
    def forward(self, x, masks_input, class_p):
        ...
        masks_inverse=torch.from_numpy(np.repeat((1-self.masks.cpu().numpy())[:, :, np.newaxis,:], 3, axis=2)).to(self.device).detach()
        
        # Generate the random Gaussian noise
        random_whole=torch.randn([N]+list((3,H,W))).to(self.device).detach()* 0.1
        
        # Define tensors holding the mask images with noise and original images with noise 
        mask_image_with_noise=torch.empty((N,3,H,W)).to(self.device).detach()
        original_image_with_noise=torch.empty((N,3,H,W)).to(self.device).detach()
        ...
        stack_whole=torch.cat((mask_image_with_noise, original_image_with_noise), 0).to(self.device).detach()
        p_whole = []
        for i in range(0, 2*N, self.gpu_batch):
            p_whole.append(self.model(stack_whole[i:min(i + self.gpu_batch, 2*N)]).detach())
